  ...
  return (
    <section className="market-board" id="markets" aria-label="Featured betting markets">
      <div className="market-board-header">
        ...
        <span className="market-note">Supabase-ready market feed</span>
      </div>
...
function IntegrationBanner() {
  ...
  return (
    <section className="integration-banner" aria-label="Integration readiness">
      <div>
        <Badge tone={supabaseReady ? 'success' : 'warning'}>{supabaseReady ? 'Supabase configured' : 'MVP readiness'}</Badge>
        <h2>{supabaseReady ? 'Backend data layer is configured for Supabase.' : 'Frontend lobby is ready; production integrations are pending.'}</h2>
        <p>{missing.length ? `Still pending for production: ${missing.join(', ')}.` : 'Environment variables are present for the configured integrations.'}</p>
      </div>
